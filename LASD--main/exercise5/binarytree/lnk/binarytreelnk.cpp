...
template<typename Data>
typename BinaryTreeLnk<Data>::NodeLnk* BinaryTreeLnk<Data>::AuxFunAssignment(NodeLnk& node){
    NodeLnk *tmp = new NodeLnk();
    tmp->element = node.Element();
    if(node.HasLeftChild()){
        tmp->left = AuxFunAssignment(node.LeftChild());
    }
    if(node.HasRightChild()) {
        tmp->right = AuxFunAssignment(node.RightChild());
    }
    return tmp;
}
/* ************************************************************************** */
